package com.zerone.myapp.web.rest;
...
import com.zerone.myapp.IntegrationTest;
import com.zerone.myapp.config.EmbeddedKafka;
import com.zerone.myapp.config.KafkaSseConsumer;
import com.zerone.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BankingExMicroKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/banking-ex-micro-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/banking-ex-micro-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/banking-ex-micro-kafka/unregister"));
                return;
            }
        }
